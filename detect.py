...
# MediaPipe 1.x no longer exposes mp.solutions at the package root.
# Import the legacy Solutions API from the compatibility package when available.
try:
    from mediapipe.python.solutions import hands as mp_hands
    from mediapipe.python.solutions import pose as mp_pose
    from mediapipe.python.solutions import drawing_utils as mp_draw
except (ImportError, ModuleNotFoundError):
    # Compatibility with MediaPipe 0.10.x
        ...
        while True:
            ...
            if hr.multi_hand_landmarks:
                for hand in hr.multi_hand_landmarks:
                    ...
                    labels.append(classify_hand(hand.landmark))
            ...
            cv2.rectangle(frame, (12, 12), (520, 105), (20, 20, 20), -1)
            cv2.putText(frame, f"FPS {fps:4.1f}", (28, 48),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 120), 2)
            cv2.putText(frame, " | ".join(dict.fromkeys(labels)) or "READY",
                        (28, 84), cv2.FONT_HERSHEY_SIMPLEX, 0.65,
                        (0, 255, 120), 2)
